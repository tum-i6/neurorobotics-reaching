...
class ddpg_agent:
    ...
    def learn(self):
        ...
        for epoch in range(self.args.n_epochs): # 
            ...
            for cycles_times in range( self.args.n_cycles):  #
                ...
                for rollouts_times in range(self.args.num_rollouts_per_mpi): 
                    ...
                    for t in range(int(self.env_params['max_timesteps'])):
                        ...
                        if epoch >= 100:
                            action = np.clip(action, -0.15, 0.15) # clip the extra action
                ...
                mb_obs = np.array(mb_obs) # (2, 101, 12)
    ...
    def _preproc_inputs(self, obs, g):
        ...
        inputs = np.concatenate([obs_norm, g_norm]) # [27+3]
        inputs = torch.tensor(inputs, dtype=torch.float32).unsqueeze(0) #  [[27+3]]
    ...
    def _soft_update_target_network(self, target, source):
        for target_param, param in zip(target.parameters(), source.parameters()): # tuple
